...
from graph.mt_graph import MTGraph
...
router = APIRouter(
    prefix="/mt",
    tags=["Merge Translator"],
)


@router.post("/check")
def chunk_checker(data: RuleData):
    """
    {
        "uuid":"12345",
        "token":"token_string",
    ...
    # values like the model and UUID into the graph at runtime.
    ...
    mt_app = mt_graph.compile(memory)
    ...
    # Invoke the graph to start the merge process. The input dictionary
    # provides the necessary data for the graph's starting nodes.
    result = mt_app.invoke(
        input=input_state,
        config=config,
    )
    # --- 4. Storing Results ---
    ...
    return {"uuid": data.uuid, "status": HTTPException(status_code=200, detail="done")}
